...
export default async function handler(req, res) {
  ...
  if (path.startsWith('/admin/')) {
    ...
    if ((m = matchPath('/admin/reset/:table', path)) && method === 'POST') {
      ...
      const seedMap = {
        users:    async () => { await db.batch([{sql:"DELETE FROM users"}, ...usersData.map(u => ({sql:"INSERT INTO users (id,name,email,password,role,avatar,bio,created_at) VALUES (?,?,?,?,?,?,?,?)",args:[u.id,u.name,u.email,u.password,u.role,u.avatar||"",u.bio||"",u.createdAt]}))]); return usersData.length },
        products: async () => { await db.batch([{sql:"DELETE FROM products"}, ...productsData.map(p => ({sql:"INSERT INTO products (id,name,description,price,category_id,stock,sku,images,created_at) VALUES (?,?,?,?,?,?,?,?,?)",args:[p.id,p.name,p.description||"",p.price,p.categoryId||"1",p.stock||0,p.sku||"",JSON.stringify(p.images||[]),p.createdAt]}))]); return productsData.length },
        posts:    async () => { await db.batch([{sql:"DELETE FROM posts"}, ...postsData.map(p => ({sql:"INSERT INTO posts (id,title,content,excerpt,tags,status,user_id,cover_image,created_at) VALUES (?,?,?,?,?,?,?,?,?)",args:[p.id,p.title,p.content,p.excerpt||"",JSON.stringify(p.tags||[]),p.status||"published",p.userId||"1",p.coverImage||"",p.createdAt]}))]); return postsData.length },
        students: async () => { await db.batch([{sql:"DELETE FROM students"}, ...studentsData.map(s => ({sql:"INSERT INTO students (id,student_id,name,gender,created_at) VALUES (?,?,?,?,?)",args:[s.id,s.studentId||"",s.name,s.gender||"",s.createdAt]}))]); return studentsData.length },
        movies:   async () => { await db.batch([{sql:"DELETE FROM movies"}, ...moviesData.map(mv => ({sql:"INSERT INTO movies (id,title,description,genre,year,rating,director,cast,duration,language,poster,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",args:[mv.id,mv.title,mv.description||"",JSON.stringify(mv.genre||[]),mv.year||null,mv.rating||null,mv.director||"",JSON.stringify(mv.cast||[]),mv.duration||null,mv.language||"English",mv.poster||"",mv.createdAt]}))]); return moviesData.length },
        books:    async () => { await db.batch([{sql:"DELETE FROM books"}, ...booksData.map(b => ({sql:"INSERT INTO books (id,title,author,isbn,genre,description,year,pages,rating,publisher,language,cover,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",args:[b.id,b.title,b.author,b.isbn||"",b.genre||"General",b.description||"",b.year||null,b.pages||null,b.rating||null,b.publisher||"",b.language||"English",b.cover||"",b.createdAt]}))]); return booksData.length },
        countries:async () => { await db.batch([{sql:"DELETE FROM countries"}, ...countriesData.map(c => ({sql:"INSERT INTO countries (id,name,code,capital,region,population,area,currency,language,flag) VALUES (?,?,?,?,?,?,?,?,?,?)",args:[c.id,c.name,c.code,c.capital||"",c.region||"",c.population||0,c.area||0,c.currency||"",c.language||"",c.flag||""]}))]); return countriesData.length },
        todos:    async () => { await db.batch([{sql:"DELETE FROM todos"}, ...todosData.map(t => ({sql:"INSERT INTO todos (id,title,description,priority,completed,due_date,created_at) VALUES (?,?,?,?,?,?,?)",args:[t.id,t.title,t.description||null,t.priority,t.completed?1:0,t.dueDate||null,new Date().toISOString()]}))]); return todosData.length },
        recipes:  async () => { await db.batch([{sql:"DELETE FROM recipes"}, ...recipesData.map(r => ({sql:"INSERT INTO recipes (id,title,description,ingredients,steps,category,prep_time,cook_time,servings,difficulty,calories,tags,image,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",args:[r.id,r.title,r.description||"",JSON.stringify(r.ingredients||[]),JSON.stringify(r.steps||[]),r.category||"Other",r.prepTime||0,r.cookTime||0,r.servings||1,r.difficulty||"easy",r.calories||0,JSON.stringify(r.tags||[]),r.image||"",r.createdAt]}))]); return recipesData.length },
