...
export default async function handler(req, res) {
  ...
  if (path === '/auth/login' && method === 'POST') {
    ...
    if (!email || !password) return _err( 400, 'email and password are required')
    const r = await db.execute({ sql: 'SELECT * FROM users WHERE email=?', args: [email] })
    if (!r.rows.length) return _err( 401, 'Invalid email or password')
    const user = toCamel(r.rows[0])
    const valid = await bcrypt.compare(password, user.password)
    if (!valid) return _err( 401, 'Invalid email or password')
    const payload = { sub: user.id, email: user.email, role: user.role }
    const accessToken  = signAccess(payload)
    const refreshToken = signRefresh(payload)
    return _ok( { accessToken, refreshToken, tokenType: 'Bearer', expiresIn: 900, user: safeUser(user) })
  }

  if (path === '/auth/register' && method === 'POST') {
    const { name, email, password, confirmPassword } = req.body || {}
    if (!name || !email || !password || !confirmPassword) return _err( 400, 'name, email, password, and confirmPassword are required')
    if (password !== confirmPassword) return _err( 400, 'Passwords do not match')
    if (password.length < 8) return _err( 400, 'Password must be at least 8 characters')
    const exists = await db.execute({ sql: 'SELECT id FROM users WHERE email=?', args: [email] })
    if (exists.rows.length) return _err( 409, 'Email already registered')
    ...
    return _ok( { user: newUser, accessToken: signAccess(p2), refreshToken: signRefresh(p2) }, 201)
  }

  if (path === '/auth/forgot-password' && method === 'POST') {
    const { email } = req.body || {}
    if (!email) return _err( 400, 'email is required')
    return _ok( { message: 'If an account with that email exists, a reset link has been sent.' })
  }
  ...
  if (path === '/users') {
    if (method === 'GET') {
      ...
      return _ok( paginate(rows, page, limit))
    }
    if (method === 'POST') {
      const { name, email, password, role = 'user', avatar } = req.body || {}
      if (!name || !email || !password) return _err( 400, 'name, email, and password are required')
      const exists = await db.execute({ sql: 'SELECT id FROM users WHERE email=?', args: [email] })
      if (exists.rows.length) return _err( 409, 'Email already exists')
      const hashed = await bcrypt.hash(password, 10)
      const id = await nextId('users')
      ...
      return _ok( { id, name, email, role, avatar: av, bio: '' }, 201)
    }
  }
  ...
  if ((m = matchPath('/users/:id', path))) {
    ...
    if (!r.rows.length) return _err( 404, 'User not found')
    const user = toCamel(r.rows[0])
    if (method === 'GET') return _ok( safeUser(user))
    if (method === 'PUT') {
      const { name, email, avatar, bio, role } = req.body || {}
      if (!name || !email) return _err( 400, 'name and email are required')
      ...
      return _ok( { ...safeUser(user), name, email, avatar: avatar||user.avatar, bio: bio??user.bio, role: role||user.role })
    }
    if (method === 'PATCH') {
  ...
  if (path === '/products') {
    ...
    if (method === 'POST') {
      ...
      if (!name || price === undefined) return _err( 400, 'name and price are required')
      ...
      return _ok( { id, name, description: description||'', price: Number(price), categoryId: '1', stock: Number(stock), sku }, 201)
    }
  }
  ...
  if ((m = matchPath('/products/:id', path))) {
    ...
    if (!r.rows.length) return _err( 404, 'Product not found')
    const prod = toCamel(r.rows[0], ['images'])
    if (method === 'GET') return _ok( prod)
    if (method === 'PUT') {
      ...
      return _ok( { ...prod, name, price: Number(price), description: description??prod.description, stock: stock!==undefined?Number(stock):prod.stock })
    }
    if (method === 'PATCH') {
  ...
  if (path === '/posts') {
    ...
    if (method === 'POST') {
      ...
      if (!title || !content) return _err( 400, 'title and content are required')
      ...
      return _ok( { id, title, content, excerpt, tags: tags||[], status: 'published', userId: '1' }, 201)
    }
  }
  ...
  if (path === '/todos') {
    ...
    if (method === 'POST') {
      ...
      if (!title?.trim()) return _err( 400, 'title is required')
      ...
      return _ok( { id, title: title.trim(), description: description||null, priority, completed: false, dueDate: dueDate||null }, 201)
    }
  }

  if ((m = matchPath('/todos/:id/toggle', path)) && method === 'PATCH') {
    const r = await db.execute({ sql: 'SELECT * FROM todos WHERE id=?', args: [m.id] })
    if (!r.rows.length) return _err( 404, 'Todo not found')
    ...
    return _ok( { ...todo, completed: newVal===1 })
  }
  ...
  if (path === '/countries') {
    ...
    if (method === 'POST') {
      ...
      return _ok( { id, name, code: code.toUpperCase(), capital: capital||'', region: region||'', population: population||0, area: area||0, currency: currency||'', language: language||'', flag: flag||'' }, 201)
    }
  }

  if ((m = matchPath('/countries/:code', path))) {
    const r = await db.execute({ sql: 'SELECT * FROM countries WHERE code=?', args: [m.code.toUpperCase()] })
    if (!r.rows.length) return _err( 404, 'Country not found')
    const country = toCamel(r.rows[0])
    if (method === 'GET') return _ok( country)
    if (method === 'PUT') {
      const { name, capital, region, population, area, currency, language, flag } = req.body || {}
      if (!name) return _err( 400, 'name is required')
      ...
      return _ok( { ...country, name, capital: capital??country.capital })
    }
    if (method === 'PATCH') {
  ...
  if (path === '/movies') {
    ...
    if (method === 'POST') {
      ...
      if (!title) return _err( 400, 'title is required')
      ...
      return _ok( { id, title, description: description||'', genre: genre||[], year: year||null, rating: rating||null, director: director||'', cast: cast||[], duration: duration||null, language: language||'English' }, 201)
    }
  }

  if ((m = matchPath('/movies/:id', path))) {
    const r = await db.execute({ sql: 'SELECT * FROM movies WHERE id=?', args: [m.id] })
    if (!r.rows.length) return _err( 404, 'Movie not found')
    const movie = toCamel(r.rows[0], ['genre','cast'])
    if (method === 'GET') return _ok( movie)
    if (method === 'PUT') {
      const { title, genre, year, rating, director, cast, duration, language, description } = req.body || {}
      if (!title) return _err( 400, 'title is required')
      ...
      return _ok( { ...movie, title, description: description??movie.description })
    }
    if (method === 'PATCH') {
  ...
  if (path === '/books') {
    ...
    if (method === 'POST') {
      ...
      if (!title || !author) return _err( 400, 'title and author are required')
      if (isbn) {
        const exists = await db.execute({ sql: 'SELECT id FROM books WHERE isbn=?', args: [isbn] })
        if (exists.rows.length) return _err( 409, 'ISBN already exists')
      }
      ...
      return _ok( { id, title, author, isbn: isbn||'', genre: genre||'General' }, 201)
    }
  }

  if ((m = matchPath('/books/:isbn', path))) {
    const r = await db.execute({ sql: 'SELECT * FROM books WHERE isbn=?', args: [m.isbn] })
    if (!r.rows.length) return _err( 404, 'Book not found')
    const book = toCamel(r.rows[0])
    if (method === 'GET') return _ok( book)
    if (method === 'PUT') {
  ...
  if (path === '/recipes') {
    ...
    if (method === 'POST') {
      ...
      return _ok( { id, title, category: category||'Other', difficulty: difficulty||'easy' }, 201)
    }
  }

  if ((m = matchPath('/recipes/:id', path))) {
    const r = await db.execute({ sql: 'SELECT * FROM recipes WHERE id=?', args: [m.id] })
    if (!r.rows.length) return _err( 404, 'Recipe not found')
    const recipe = toCamel(r.rows[0], ['ingredients','steps','tags'])
    if (method === 'GET') return _ok( recipe)
    if (method === 'PUT') {
      const { title, description, ingredients, steps, category, prepTime, cookTime, servings, difficulty, calories, tags } = req.body || {}
      if (!title) return _err( 400, 'title is required')
  ...
  if (path === '/animals') {
    ...
    if (method === 'POST') {
      ...
      return _ok( { id, name, category: category||'Mammal', conservationStatus: conservationStatus||'Least Concern' }, 201)
    }
  }

  if ((m = matchPath('/animals/:id', path))) {
    const r = await db.execute({ sql: 'SELECT * FROM animals WHERE id=?', args: [m.id] })
    if (!r.rows.length) return _err( 404, 'Animal not found')
    const animal = toCamel(r.rows[0])
    if (method === 'GET') return _ok( animal)
    if (method === 'PUT') {
      const { name, scientificName, category, habitat, diet, lifespan, weight, length: len, conservationStatus, description } = req.body || {}
      if (!name) return _err( 400, 'name is required')
  ...
  if (path === '/students') {
    ...
    if (method === 'POST') {
      ...
      if (studentId) {
        ...
        if (exists.rows.length) return _err( 409, 'studentId already exists')
      }
      ...
      return _ok( { id, studentId: studentId?String(studentId):'', name, gender: gender||'' }, 201)
    }
  }

  if ((m = matchPath('/students/:id', path))) {
    const r = await db.execute({ sql: 'SELECT * FROM students WHERE id=?', args: [m.id] })
    if (!r.rows.length) return _err( 404, 'Student not found')
    const student = toCamel(r.rows[0])
    if (method === 'GET') return _ok( student)
    if (method === 'PUT') {
      const { studentId, name, gender } = req.body || {}
      if (!name) return _err( 400, 'name is required')
      ...
      return _ok( { ...student, name, gender: gender??student.gender })
    }
    if (method === 'PATCH') {
      const setCols = []; const args = []
  ...
  if (path === '/admin/login' && method === 'POST') {
    ...
    if (password !== ADMIN_PASSWORD) return _err( 401, 'รหัสผ่านไม่ถูกต้อง')
    return _ok( { token: Buffer.from(`admin:${ADMIN_PASSWORD}:${Date.now()}`).toString('base64') })
  }
  ...
  if (path.startsWith('/admin/')) {
    ...
    if (!token) return _err( 401, 'กรุณา login ก่อน')
    try {
      const decoded = Buffer.from(token, 'base64').toString('utf8')
      if (!decoded.startsWith(`admin:${ADMIN_PASSWORD}:`)) return _err( 401, 'token ไม่ถูกต้อง')
    } catch { return _err( 401, 'token ไม่ถูกต้อง') }
    ...
    if (path === '/admin/stats' && method === 'GET') {
      ...
      return _ok( { rowCounts: counts, totalLogs: logCount, recentActivity: recentLogs })
    }
    ...
    if ((m = matchPath('/admin/reset/:table', path)) && method === 'POST') {
      ...
      if (!tableName) return _err( 400, `ไม่รู้จัก table: ${m.table}`)

      // เรียก seed endpoint ภายใน
      const { execFile } = await import('child_process')
      const { promisify } = await import('util')
      const execFileAsync = promisify(execFile)
      try {
        await execFileAsync('node', ['scripts/seed-table.js', tableName], { cwd: '/var/task' })
      } catch (e) {
        return _err( 500, `reset ล้มเหลว: ${e.message}`)
      }
      await db.execute({
        sql: 'INSERT INTO activity_logs (method, path, body, ip, status_code, timestamp) VALUES (?,?,?,?,?,?)',
        args: ['POST', `/api/admin/reset/${m.table}`, null, 'admin', 200, new Date().toISOString()],
      })
      return _ok( { message: `reset ${m.table} สำเร็จแล้ว` })
    }

    return _err( 404, `Admin route not found: ${method} /api${path}`)
  }

  // ── 404 ────────────────────────────────────────────────────────────────
  return _err( 404, `Route not found: ${method} /api${path}`)
}
